...
def test_2_1_cpu_detection() -> None:
    ...
    if slurm_cpus is None:
        record(True, "2.1 CPU detection",
               f"SLURM_CPUS_PER_TASK not set (not a SLURM job?), workers={workers}")
    else:
        record(workers == int(slurm_cpus),
               "2.1 CPU detection",
               f"workers={workers}, expected={slurm_cpus}")
...
def test_2_2_job_array_detection() -> None:
    ...
    if slurm_tid is None or slurm_cnt is None:
        record(True, "2.2 Job array detection",
               "Not in a job array; detect_job_array() should return None")
        if arr is not None:
            record(False, "2.2 Job array detection",
                   f"Expected None but got {arr}")
    else:
...
def test_2_3_job_array_iteration_split() -> None:
    ...
    for item in my_slice:
        if item % task_count != task_id:
            record(False, "2.3 Job array split",
                   f"Item {item} assigned to task {task_id} (mod {task_count})")
            return

    print(f"[INFO] Task {task_id}/{task_count}: slice = {my_slice}")
    record(True, "2.3 Job array iteration split",
           f"Task {task_id}/{task_count}: {len(my_slice)} items, no overlap")
...
def test_2_4_blas_thread_limiting() -> None:
    ...
    record(ok, "2.4 BLAS thread limiting",
           "All thread variables should be set to '1'")
