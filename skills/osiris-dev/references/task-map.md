...
- **Adding a new diagnostic type without wiring all 6 points.** A new analyzer/vis pair requires: (1) `analysis/` class + `_result_types` dataclass (2) `analysis/__init__.py` hub property (3) `vis/` plot function (4) `vis/__init__.py` import + staticmethod + `_XVis` namespace + `plot()` dispatch + `invalidate_cache` + `__all__` (5) `batch.py` directory + processing block + docstring update (6) `conftest.py` fixture. Missing any one breaks the Hub or batch paths silently.
- **Changing `_parse_iter_file` signature without grep.** The return type is unpacked at 8+ call sites across `simulation.py`. Changing it without updating every caller produces `ValueError: too many values to unpack` at runtime.
