...
- 20 unit tests for `resource._estimator.py` covering internal formulas (memory, runtime, disk)
- Dual-track installation documentation (uv + pip/venv) in all 6 developer docs

### Fixed
- `field_energy_all` dict key renamed `"energy"` → `"total_energy"` to match `FieldEnergyResult`
- Parallel batch path now returns populated `BatchResult` (files, elapsed, errors) instead of empty stub
- Flat `{quant}-{species}-{iter:06d}.zdf` filenames now correctly discovered for DENSITY/RAW/PHA diagnostics
- `_parse_iter_file` regex changed from greedy `(.+)` to non-greedy `([^-]+?)` — species no longer baked into quantity

### Changed
- `Makefile` and `.pre-commit-config.yaml` decoupled from `uv` — commands work with any activated venv
- Pre-commit hooks now work on Windows without uv installed
...
[0.17.0]: https://github.com/wulnkkk/osiris-toolkit/compare/v0.16.0...v0.17.0
