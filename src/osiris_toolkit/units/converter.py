...
def _build_scales(omega_p: float) -> dict[str, dict[str, float]]:
    ...
    return {
        ...
        "momentum": {
            "norm": 1.0,
            "kg*m/s": M_ELECTRON * C_LIGHT,
            "MeV/c": M_ELECTRON * C_LIGHT * C_LIGHT / (E_CHARGE * 1e6),
        },
        "energy": {
            "norm": 1.0,
            "J": M_ELECTRON * C_LIGHT**2,
...
class UnitConverter:
    ...
    def get_scale(self, quantity: str, unit: str) -> float:
        ...
        if q_scales is None:
            raise UnitConversionError(f"Unknown quantity {quantity!r}. Known: {sorted(self._scales)}")
        if unit == "auto":
            unit = _AUTO_UNITS.get(quantity, "norm")
        s = q_scales.get(unit)
        if s is None:
            raise UnitConversionError(f"Unknown unit {unit!r} for {quantity}. Known: {sorted(q_scales)}")
        return s
    ...
    def __repr__(self) -> str:
        return f"UnitConverter(omega_p={self.omega_p:.4g} rad/s, n0={self.n0_cm3:.4g} cm^-3)"
    ...
    def __getitem__(self, name: str):
        ...
        if hasattr(self, name):
            ...
            if isinstance(q, QuantityKind):
                return q
        raise UnitConversionError(f"Unknown quantity {name!r}. Known: {[q.name for q in self.quantities]}")

    def __repr__(self) -> str:
        return f"UnitSystem(omega_p={self.omega_p:.4g} rad/s)"
